...
现实工作中，普通用户是通过钱包中的投票选项来选择见证者的。在全网出现不诚实区块的时候，正常的钱包可能在交易以前强制举行一轮新投票。
...
- 见证人的位置有上一个区块的最后部分随机决定。实际上在 EOS 等实现中，这可能是个定制拓扑的 round robin 结构，每个出块者都可以跟下一个出块者直连以防块扩散速度不足。而且这种定制的 round robin 顺序可能还会被洗牌算法所改变。
